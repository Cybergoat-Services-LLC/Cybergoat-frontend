...
- `/desc-isr-readiness`, `/desc-isr-readiness/submitted` — **NEW 2026-08-08, built against a contract, not yet verified end-to-end.** Public, unauthenticated top-of-funnel lead-gen tool: visitor uploads security policy docs (PDF/DOCX, 20MB/file client-validated), gets a free AI-assisted DESC ISR readiness score, human-reviewed before email delivery (1-2 business days, no instant result). Proxies to backend via new Route Handlers `src/app/api/compliance-assessor/{submit,submissions/[id]/status}/route.ts` — `submit` is the **only** Route Handler in this codebase that intentionally skips `getPortalToken()`, since it's a public flow; both are rate-limited via `isRateLimited()` as defense-in-depth against per-submission AI cost. Homepage CTA banner added between `GRCSection` and `CorporateB2BSection` in `sections.tsx`/`page.tsx`. **Built against a backend contract (`POST /v1/compliance-assessor/submit`, `GET /v1/compliance-assessor/submissions/{id}/status`) that a backend agent was building in parallel and wasn't reachable in this dev environment — real submission was never tested end-to-end, only form validation/error-state rendering.** Also fixed a latent bug in `callPortalApi` (`lib/portalAuth.ts`) that force-overwrote `Content-Type` to `application/json` on any request with a body, which would have silently broken this (and any future) multipart file upload. Committed locally only, not deployed — pending user review of both the frontend build and the backend contract match.
...
- **DESC ISR Readiness Score** (added 2026-08-08, built locally, **NOT deployed** — awaiting owner review + separate website changes): public lead-gen tool. Visitor uploads policy PDFs/DOCX at `POST /v1/compliance-assessor/submit` (throttled `3,60` — every submission costs real Gemini/Upstash money), documents are chunked into a **per-submission Upstash Vector namespace** (`submission-{id}`, path-level isolation, not metadata filtering), Gemini scores them against 23 ISR main controls, and an SME reviews and approves in Filament before anything is emailed. **No code path sends a report without `approved_by` set** — enforced in `ComplianceReportService`, not just the UI, with tests that actively try to bypass it. Needs `UPSTASH_VECTOR_REST_URL`/`TOKEN` before it can run at all (fails loudly with "Upstash Vector not configured", never silently). New tables: `compliance_controls` (86 verbatim ISR requirements), `compliance_submissions`, `compliance_assessment_results`.
...
Backend: `C:\php\php.exe artisan test` — **239 tests passing** as of 2026-08-08 (was 175 on 2026-08-07; +64 from the ISR Readiness Score build), zero known regressions.

**Do not run `vendor/bin/pint`.** This repo is not Pint-clean and never has been — a `--test` run flags ~80 pre-existing files for `not_operator_with_successor_space`, `concat_space` and friends. The house style genuinely is `!$x` and `'a' . 'b'`. Running Pint would reformat the entire codebase into one unreviewable diff. Match surrounding code instead. Full suite re-run after every change this session; treat a red suite as blocking.
...
- **Upstash Vector provisioning** — now blocking *two* features, not one: the RAG chatbot (below) and the DESC ISR Readiness Score assessor (§4). Separate product from the Redis already in use. Needs `UPSTASH_VECTOR_REST_URL`/`TOKEN` in the Laravel backend env (for the assessor) and `.env.local` + Vercel (for the chatbot). The assessor's index must be created **with a built-in embedding model**, since it uses Upstash's `/upsert-data` + `/query-data` endpoints rather than supplying its own vectors.
- **RAG chatbot** (replacing the manual `/admin/training` Q&A entry with real semantic search over course/FAQ content): blocked on the Upstash Vector provisioning above.
- **ISR Readiness Score deploy**: built and committed locally 2026-08-08, deliberately **not deployed** — the owner wanted to review the build and make separate website changes first. When it does go out it adds 3 migrations, so the `migrate-cybergoat-lms` job needs its usual image update + execute (§3). It also needs a public front end (no page exists yet — the API endpoint and `GET /v1/compliance-assessor/limits` are ready for one). **Verify `MAIL_MAILER` on the Cloud Run service before relying on delivery**: the repo's `.env`/`.env.example` both say `log`, and this Mailable is the backend's first — no email has ever actually been sent from the Laravel side, so the Google Workspace SMTP path is unproven here even if the deployed env has credentials set.
